...
const fetchList = async () => {
  loading.value = true
  try { const res = await post('/role/list', {}); if (res.code === 200) tableData.value = res.data.list } catch (e) { /* */ }
...
const handleDelete = (row) => {
  ElMessageBox.confirm(`确定删除角色 "${row.name}" 吗？`, '提示', { type: 'warning' }).then(async () => {
    const res = await post('/role/delete', { id: row.id })
...
const handleSubmit = async () => {
  ...
  await formRef.value.validate(async (valid) => {
    ...
    if (!valid) return; submitLoading.value = true; try { const res = await post(isEdit.value ? '/role/update' : '/role/add', data)
...
const handleSavePermission = async () => {
  ...
  try {
    const res = await post('/permission/role/update', { role_id: permRoleId.value, permission_ids: checkedKeys })
