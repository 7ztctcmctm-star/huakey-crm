...
const fetchSalesUsers = async () => { if (!isBoss.value) return; try { const r = await getSalesUsers(); if (r.code===200) salesUsers.value = r.data } catch{} }

const handleBatchAssign = async (ownerId) => {
  if (!ownerId || selectedRows.value.length===0) return
  try {
    const r = await batchAssignCustomer({ customer_ids: selectedRows.value.map(r=>r.id), to_user_id: ownerId, remark: '线索批量分配' })
...
const openDetail = async (row) => {
  ...
  try { const r = await getCustomerDetail(row.id); if (r.code===200) { detailLead.value = r.data.customer || r.data; followList.value = r.data.followRecords || [] } } catch { ElMessage.error('加载详情失败') }
}

const closeDetail = () => { detailLead.value = null; followList.value = [] }

const updateFollowStatus = async (val) => {
  try { const r = await updateCustomer({ id: detailLead.value.id, follow_status: val }); if (r.code===200) { ElMessage.success('状态已更新'); fetchList(); fetchStats() } } catch { ElMessage.error('更新失败') }
}
...
const addFollow = async () => {
  ...
  try {
    const r = await addFollowUp({ customer_id: detailLead.value.id, follow_type: followForm.follow_type, content: followForm.content, next_time: followForm.next_time || null })
    if (r.code===200) {
      ElMessage.success('跟进记录已保存')
      followForm.content = ''; followForm.next_time = ''
      // 刷新详情和列表
      const r2 = await getCustomerDetail(detailLead.value.id)
...
const fetchStats = async () => { try { const r = await getLeadsStats(); if (r.code===200) Object.assign(stats, r.data) } catch{} }
...
const fetchList = async () => {
  ...
  try {
    ...
    const r = await getLeadsList(params)
...
const handleClaim = async (row) => { const r = await claimLead(row.id); if (r.code===200) { ElMessage.success('领取成功'); tableData.value = tableData.value.filter(t=>t.id!==row.id); fetchStats() } }
const handleConvert = (row) => {
  ElMessageBox.confirm(`确定将「${row.company_name}」转化为正式客户吗？`, '转化确认', { type: 'warning' })
    .then(async () => {
      const r = await convertLead(row.id)
      if (r.code === 200) {
        ElMessageBox.alert(
...
const handleMarkLost = (row) => { ElMessageBox.confirm(`确定标记为已流失吗？`, '确认', { type: 'warning' }).then(async () => { const r = await markLeadLost(row.id); if (r.code===200) { ElMessage.success('已标记'); fetchList(); fetchStats() } }).catch(()=>{}) }
const handleSubmit = async () => { if (!formRef.value) return; await formRef.value.validate(async (valid) => { if (!valid) return; saveLoading.value = true; try { const data = { ...form, status: 1 }; if (isEdit.value) data.id = editId.value; const r = isEdit.value ? await updateCustomer(data) : await addCustomer(data); if (r.code===200) { ElMessage.success(isEdit.value?'修改成功':'新增成功'); dialogVisible.value = false; fetchList(); fetchStats() } } finally { saveLoading.value = false } }) }
