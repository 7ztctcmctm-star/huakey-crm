...
async function listRecords(pool, params = {}) {
  ...
  const [rows] = await pool.query(`
    SELECT s.id, s.customer_id, s.contact_id, s.platform, s.direction, s.content, s.attachment_url, s.message_time, s.create_by, s.create_time, s.deleted_at,
...
async function createRecord(pool, data, userId) {
  ...
  if (!platform || !direction) {
    throw new AppError(ErrorCodes.VALIDATION_ERROR, '平台和方向必填')
  }
...
async function deleteRecord(pool, id, userId, manageAll) {
  ...
  if (!record) {
    throw new AppError(ErrorCodes.RECORD_NOT_FOUND, '记录不存在')
  }
  if (record.create_by !== userId && !manageAll) {
    throw new AppError(ErrorCodes.PERMISSION_DENIED, '无权删除此记录')
  }
...
async function getCustomerTimeline(pool, customerId) {
  const [rows] = await pool.query(`
    SELECT s.id, s.customer_id, s.contact_id, s.platform, s.direction, s.content, s.attachment_url, s.message_time, s.create_by, s.create_time, s.deleted_at,
           ct.name as contact_name, u.real_name as create_by_name
    FROM crm_social_contact s
    LEFT JOIN crm_contact ct ON s.contact_id = ct.id
    LEFT JOIN sys_user u ON s.create_by = u.id
