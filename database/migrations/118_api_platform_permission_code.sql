...
-- 1. 创建权限码（主码 + 操作级子码）
INSERT IGNORE INTO sys_permission (code, name, type, parent_id, sort_order, status) VALUES
('api_platform',         'API 平台管理',    'module', NULL, 60, 1);
...
INSERT IGNORE INTO sys_permission (code, name, type, parent_id, sort_order, status) VALUES
('api_platform:view',    '查看 API 密钥/Webhook', 'action', @api_platform_id, 1, 1),
('api_platform:add',    '创建 API 密钥/Webhook', 'action', @api_platform_id, 2, 1),
('api_platform:edit',    '编辑 API 密钥/Webhook', 'action', @api_platform_id, 3, 1),
('api_platform:delete',  '删除 API 密钥/Webhook', 'action', @api_platform_id, 4, 1);
