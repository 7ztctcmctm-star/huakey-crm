...
--
-- 修订说明（2026-09-12，真库实测）：
--   原写法为
--     SELECT IF(DATABASE() NOT LIKE '%test%' AND DATABASE() NOT LIKE '%dev%',
--       (SELECT `ABORT__NOT_A_TEST_DATABASE`), 'test_db_ok') AS `guard`;
--   该写法**在 MySQL 下根本无法工作**：MySQL 在解析阶段就校验标识符，
--   `ABORT__NOT_A_TEST_DATABASE` 列不存在 → **任何库上都直接报
--   ERROR 1054 Unknown column**，与运行时的 IF 分支无关。
--   即：所谓「环境守卫」从未真正生效过，它只是制造了一条必然的报错。
--
--   改用 PREPARE/EXECUTE 动态 SQL —— 非法语句只在**满足条件时**才被解析，
--   因此既能在测试库上静默通过，又能在非测试库上真正中止。
SET @guard_sql = IF(
...
-- 说明（2026-09-12 修订）：原文件此处为 `USE huakey_crm;`，与上一行的测试库守卫自相矛盾，
...
-- 列说明（2026-09-12）：status / business_status 均改用当前字符串枚举。
--   历史数字映射依据迁移 070（旧 1→following、2→signed、3→lost、5→following）；
--   business_status 依据迁移 097（following/signed/lost/lead 等）。
--   这些客户均已有 owner_id，故 pool_status 一律为 'private'。
INSERT INTO crm_customer (company_name, contact_name, phone, industry, source, level, owner_id, status, business_status, pool_status, last_follow_time, create_time) VALUES
('深圳华科科技有限公司', '赵总', '13800138001', '通信设备', '展会', 'A', @sales_wang_id, 'following', 'following', 'private', DATE_SUB(NOW(), INTERVAL 2 DAY), DATE_SUB(NOW(), INTERVAL 60 DAY)),
('广州明源电子有限公司', '钱经理', '13800138002', '电子制造', 'Facebook', 'B', @sales_wang_id, 'following', 'following', 'private', DATE_SUB(NOW(), INTERVAL 5 DAY), DATE_SUB(NOW(), INTERVAL 50 DAY)),
('珠海航宇通讯技术公司', '孙工', '13800138003', '通讯技术', 'LinkedIn', 'A', @sales_wang_id, 'following', 'following', 'private', DATE_SUB(NOW(), INTERVAL 20 DAY), DATE_SUB(NOW(), INTERVAL 90 DAY)),
('佛山顺德电器制造厂', '李厂长', '13800138004', '电器制造', '转介绍', 'B', @sales_wang_id, 'following', 'following', 'private', DATE_SUB(NOW(), INTERVAL 18 DAY), DATE_SUB(NOW(), INTERVAL 80 DAY)),
('中山市灯饰有限公司', '周采购', '13800138005', '照明行业', '独立站', 'C', @sales_wang_id, 'following', 'following', 'private', NULL, DATE_SUB(NOW(), INTERVAL 45 DAY)),
('东莞五金精密加工厂', '吴技术', '13800138006', '五金加工', '电话', 'C', @sales_wang_id, 'following', 'following', 'private', NULL, DATE_SUB(NOW(), INTERVAL 40 DAY)),
('惠州TCL配套供应商', '郑经理', '13800138007', '家电配套', '展会', 'B', @sales_wang_id, 'following', 'following', 'private', DATE_SUB(NOW(), INTERVAL 3 DAY), DATE_SUB(NOW(), INTERVAL 35 DAY)),
('汕头市澄海玩具厂', '陈总', '13800138008', '玩具制造', '其他', 'D', @sales_wang_id, 'following', 'following', 'private', NULL, DATE_SUB(NOW(), INTERVAL 30 DAY)),
('肇庆市新材料科技公司', '刘博士', '13800138009', '新材料', 'Instagram', 'B', @sales_wang_id, 'signed', 'signed', 'private', DATE_SUB(NOW(), INTERVAL 5 DAY), DATE_SUB(NOW(), INTERVAL 70 DAY)),
('江门市摩托车配件厂', '黄厂长', '13800138010', '汽摩配件', '其他网络渠道', 'C', @sales_wang_id, 'signed', 'signed', 'private', DATE_SUB(NOW(), INTERVAL 10 DAY), DATE_SUB(NOW(), INTERVAL 65 DAY));

-- 批量生成王销售其余46个普通客户（按日递减）
INSERT INTO crm_customer (company_name, contact_name, phone, industry, source, level, owner_id, status, business_status, pool_status, last_follow_time, create_time)
SELECT CONCAT('王销售客户-', n, '号公司'), CONCAT('联系人', n), CONCAT('139', 10000000 + n),
       ...
       @sales_wang_id,
       -- 原 CASE 返回数字 2/1；按迁移 070 映射为 signed/following，business_status 与之一致
       CASE WHEN n % 5 = 0 THEN 'signed' ELSE 'following' END,
       CASE WHEN n % 5 = 0 THEN 'signed' ELSE 'following' END,
       'private',
...
INSERT INTO crm_customer (company_name, contact_name, phone, industry, source, level, owner_id, status, business_status, pool_status, last_follow_time, create_time) VALUES
('北京中关村软件科技公司', '何总', '13800138020', '软件', 'Facebook', 'A', @sales_li_id, 'following', 'following', 'private', DATE_SUB(NOW(), INTERVAL 1 DAY), DATE_SUB(NOW(), INTERVAL 90 DAY)),
('上海浦东集成电路设计公司', '吕经理', '13800138021', '集成电路', '展会', 'A', @sales_li_id, 'signed', 'signed', 'private', DATE_SUB(NOW(), INTERVAL 2 DAY), DATE_SUB(NOW(), INTERVAL 85 DAY)),
('杭州西湖区互联网公司', '施总监', '13800138022', '互联网', 'LinkedIn', 'B', @sales_li_id, 'following', 'following', 'private', DATE_SUB(NOW(), INTERVAL 3 DAY), DATE_SUB(NOW(), INTERVAL 80 DAY)),
('成都高新区游戏公司', '张开发', '13800138023', '游戏', 'Instagram', 'B', @sales_li_id, 'following', 'following', 'private', DATE_SUB(NOW(), INTERVAL 1 DAY), DATE_SUB(NOW(), INTERVAL 75 DAY)),
('重庆市渝中区金融科技公司', '孔经理', '13800138024', '金融科技', '独立站', 'C', @sales_li_id, 'following', 'following', 'private', DATE_SUB(NOW(), INTERVAL 2 DAY), DATE_SUB(NOW(), INTERVAL 70 DAY));
...
-- 赵销售：28个客户（新人），3个超15天
INSERT INTO crm_customer (company_name, contact_name, phone, industry, source, level, owner_id, status, business_status, pool_status, last_follow_time, create_time) VALUES
('南京雨花台区大数据公司', '曹总', '13800138050', '大数据', '展会', 'A', @sales_zhao_id, 'following', 'following', 'private', DATE_SUB(NOW(), INTERVAL 3 DAY), DATE_SUB(NOW(), INTERVAL 20 DAY)),
('武汉光谷通信技术公司', '严经理', '13800138051', '通信', 'Facebook', 'B', @sales_zhao_id, 'following', 'following', 'private', DATE_SUB(NOW(), INTERVAL 25 DAY), DATE_SUB(NOW(), INTERVAL 25 DAY)),
('西安高新区半导体公司', '华工', '13800138052', '半导体', 'LinkedIn', 'A', @sales_zhao_id, 'following', 'following', 'private', NULL, DATE_SUB(NOW(), INTERVAL 22 DAY));
...
INSERT INTO crm_customer (company_name, contact_name, phone, industry, source, level, owner_id, status, business_status, pool_status, last_follow_time, create_time) VALUES
('厦门市思明区软件外包公司', '金经理', '13800138070', '软件外包', 'LinkedIn', 'B', @sales_chen_id, 'following', 'following', 'private', DATE_SUB(NOW(), INTERVAL 17 DAY), DATE_SUB(NOW(), INTERVAL 45 DAY)),
('福州市马尾区电子制造厂', '魏工', '13800138071', '电子制造', '电话', 'C', @sales_chen_id, 'following', 'following', 'private', NULL, DATE_SUB(NOW(), INTERVAL 50 DAY)),
('泉州市石狮服装制造厂', '陶总', '13800138072', '服装制造', '展会', 'B', @sales_chen_id, 'signed', 'signed', 'private', DATE_SUB(NOW(), INTERVAL 2 DAY), DATE_SUB(NOW(), INTERVAL 60 DAY));
...
-- 为超15天未跟进的客户生成今日提醒
-- 说明（2026-09-12）：原条件 `c.status NOT IN (2, 3) AND c.status != 0` 是旧数字语义
--   （2=已签约、3=已流失、0=已删除）。按迁移 070 的字符串枚举重写为：
--   排除终态（signed/lost）与线索（lead），只对活跃跟进阶段的客户提醒。
...
WHERE c.deleted_at IS NULL
  AND c.business_status IN ('following', 'quoted', 'negotiating')
...
-- 说明（2026-09-12）：原为 `WHERE status != 0`（旧数字语义：0=已删除）。软删除现由 deleted_at 表达。
SELECT '客户', COUNT(*) FROM crm_customer WHERE deleted_at IS NULL UNION ALL
...
-- business_status 分布（2026-09-12 新增）：seed 的关键不变量是
-- 「已签约客户必须真的落在 business_status='signed'」，否则会被正式客户列表整体过滤
SELECT '=== business_status 分布（应无意外堆积在 lead）===' AS info;
