...
3. 在 `.github/workflows/ci.yml` 中新增 `image-scan` job：
   - Build Docker image
   - 使用 `aquasecurity/trivy-action@0.28.0`
   - severity: CRITICAL 时报红但不确定阻断（exit-code: '1'）

4. 在 `frontend/playwright.config.js` 中补充移动端视口配置（已有项目，补充第二个项目）：
   ```js
   {
     name: 'Mobile Chrome',
     use: {
...
5. 在 `frontend/e2e/` 中新增 `responsive.spec.js`：
   - 仅做 smoke 级别的响应式验证
...
4. 为核心路由补充 JSDoc 注释（选 6 个最重要的）：
   ...
   每个路由至少包含 `@swagger` 注释，格式参考 swagger-jsdoc 标准。
