...
> | 编号 | 问题 | 状态 | 提交 | 验证方式 |
> |------|------|------|------|----------|
> | P0-1 | 公海认领竞态 | ✅ **已修复（含补齐）** | `e791bd6` + `6716504` | **双并发实测**：修复后 1 成功/1 失败；旧写法 2 成功（竞态复现）；新增 7 个回归用例。<br>⚠️ **重要：首次修复只覆盖了 1/3** —— 做「客户总览」第 2 步时才发现在全项目**存在三套独立的认领实现**，其中 `customerService.claimPoolCustomer` 由 `POST /pool/claim` 调用、是**活的接口**，同样存在该竞态。已在 `6716504` 补齐三套实现 |
