...
- 工程层：Vite 构建、ESLint、Vitest 单测、Playwright E2E — 基础设施齐全

但仍存在**体验断层**：骨架屏覆盖率不足 20%、加载态参差、部分页面缺少交互反馈、登录页视觉单薄、可访问性基本为零、部分组件重复代码。
...
**现状**：`components/common/StateWrapper.vue` 已封装好「加载 / 错误 / 空 / 正常」四态切换，但**全项目零引用**，属于死代码。各页面自行用 `v-if/v-else` 拼接状态，质量参差不齐。
