...
| 22 | 权限闸门 | 同上缺陷的**系统性扩散**：全库 **47 处** `requireManager` 挂载 / **13 个**路由文件；中间件语义排除 manager ⇒ 约 **35 个**「角色已持有功能权限码 + 前端菜单可见」端点对**部门经理全部 403**（hr 角色亦被挡在自己模块外） | P2（可达的「功能不可用」而非越权） | ✅ 中间件与配对已修(2026-09-20)；`hr.js`(17 端点) 待产品决策 |
