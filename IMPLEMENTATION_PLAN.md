...
- [ ] 1.3.1 `rg pg\b` 全局搜索确认 `pg` 无引用
- [ ] 1.3.2 `rg supabase` 全局搜索确认 `@supabase/supabase-js` 无引用
- [ ] 1.3.3 从 `backend/package.json` 移除 `pg` 和 `@supabase/supabase-js`
...
- [ ] 1.8.2 调整 `max_connections` → 50
- [ ] 1.8.3 开启慢查询日志 `slow_query_log = ON`, `long_query_time = 2`
- [ ] 1.8.4 调整 `innodb_log_file_size` → 128M
- [ ] 1.8.5 文档: MYSQL_CONFIG.md

### 1.9 CASCADE 删除审计

- [x] 1.9.1 审查所有 migration 文件中的 `ON DELETE CASCADE` 约束
- [ ] 1.9.2 重点审查 `crm_customer` 的级联删除链
- [ ] 1.9.3 确认业务层软删除（`deleted_at`）是否触发 DB 级联
- [ ] 1.9.4 如有风险，将 CASCADE 改为 RESTRICT + 业务层手动处理
...
- [ ] 2.1.1 后端: 新建 `backend/core/ModuleRegistry.js` — 模块注册器
- [ ] 2.1.2 后端: 模块自描述格式 `module.js` — routes + permissions + migrations
- [ ] 2.1.3 后端: `app.js` 改为遍历注册器加载路由（替代42条手工 `apiRouter.use`）
- [ ] 2.1.4 后端: 建立 service 间调用白名单机制
- [ ] 2.1.5 前端: 新建 `frontend/src/router/modules/` — 模块路由懒加载注册
- [ ] 2.1.6 前端: `router/index.js` 改为聚合模块路由（替代68条手工定义）

### 2.2 启用 Redis

- [ ] 2.2.1 `REDIS_ENABLED` 改为 `true`，验证连接
- [ ] 2.2.2 替换 `node-cache` 权限缓存为 Redis（跨进程共享）
- [ ] 2.2.3 登录限流持久化到 Redis（替代内存 Map）
- [ ] 2.2.4 session 存储从内存迁移到 Redis
- [ ] 2.2.5 Docker Compose 中 Redis 容器配置（128MB 内存限制）

### 2.3 读写分离

- [ ] 2.3.1 `config/database.js` 创建 `readOnlyPool`（使用 `DB_RO_*` 环境变量）
- [ ] 2.3.2 报表类路由（`report/*`）查询切换到 `readOnlyPool`
- [ ] 2.3.3 AI 查询路由（`ai.js`）切换到 `readOnlyPool`
- [ ] 2.3.4 搜索路由（`search.js`）切换到 `readOnlyPool`
- [ ] 2.3.5 验证: 写操作使用读写池，只读查询使用只读池

### 2.4 API 版本前缀

- [ ] 2.4.1 所有路由改为 `/api/v1/` 前缀
- [ ] 2.4.2 旧路由 `/api/` 保留转发（3个月过渡期，返回 Deprecation 头）
- [ ] 2.4.3 前端 API base URL 更新为 `/api/v1/`
- [ ] 2.4.4 文档: API_VERSIONING.md

### 2.5 大 Route 拆分

- [ ] 2.5.1 `knowledge.js` (16.6KB) — SQL 逻辑抽取到 `knowledgeService.js`
- [ ] 2.5.2 `supplier.js` (14.9KB) — 抽取到现有 `supplierService.js` 完善
- [ ] 2.5.3 `detail.js` (12KB) — 抽取到 `detailService.js`
- [ ] 2.5.4 其他超过 300 行的 route 文件 — 逐文件审计拆分

### 2.6 基础监控

- [ ] 2.6.1 接入 `nodemailer` 做关键错误邮件告警
- [ ] 2.6.2 配置告警规则: 未捕获异常、DB 连接失败、Redis 连接失败
- [ ] 2.6.3 健康检查端点 `/api/health` 增强 — 检测 DB/Redis 连通性
- [ ] 2.6.4 Docker 容器健康检查与告警联动
...
- [ ] 3.1.1 采购模块扩展 — 采购申请、比价、审批流程
- [ ] 3.1.2 字段级权限 — 隐藏/拦截敏感字段（`cost_price` 等）
- [ ] 3.1.3 消息通知中心 — 前端统一通知面板 + SSE 实时推送
- [ ] 3.1.4 报表/仪表盘增强 — 采购成本分析、供应商绩效、预算执行

### 3.2 代码质量

- [ ] 3.2.1 前端大组件拆分 — `Dashboard.vue` (963行), `layout/index.vue` (945行), `customer/List.vue` (905行)
- [ ] 3.2.2 Swagger/OpenAPI 文档自动生成
- [ ] 3.2.3 测试覆盖率提升（当前 ~75 个测试，目标 120+）

### 3.3 基础设施

- [ ] 3.3.1 `crm_pool_log` 表按 `create_time` 做 RANGE 分区
- [ ] 3.3.2 GitHub Actions CI/CD（自动测试 → Docker 构建推送）
- [ ] 3.3.3 NAS 部署增加 Nginx 反向代理容器
- [ ] 3.3.4 MySQL 内存从 512MB → 1GB，App 内存从 512MB → 384MB
