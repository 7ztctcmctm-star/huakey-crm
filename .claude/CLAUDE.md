> ## ⚠️ 动手之前必读
>
> **本项目的最高行为准则是仓库根目录的 [`AGENTS.md`](../AGENTS.md)。**
> 来源为《HUAKEYCRM 生产级全面接管、分析、补全、修复与验收任务书》。
>
> 执行任何**分析 / 修改 / 测试 / 部署**动作之前，必须先完整阅读 `AGENTS.md`。
> 与本文档冲突时以 `AGENTS.md` 为准；与实际代码冲突时以**实际代码与实际运行结果**为准。
>
> **十条红线**：先读懂再动手 · 先扫描后修改 · 九步循环不省步 · 从 P0 修起 ·
> 权限必须后端强制 · 关键业务必须有事务 · 并发必须有锁 · 删数据先想历史 ·
> 备份必须验证可恢复 · **绝不"假完成"**。
>
> 本文档记录的是本项目**既有的技术约定**，与 `AGENTS.md`（接管与验收准则）**互为补充，两者都要读**。
