...
**7. ~~Token无法撤销~~** ✅ 已修复（2026-06-22：sys_token_blacklist黑名单机制）
- ~~Logout不使token失效~~ → 登出时token hash写入黑名单
...
| ~~**P1 高危** | Token无法撤销~~ | ~~离职员工token在有效期内仍可访问~~ | ✅ 已修复（2026-06-22） |
...
| ~~9 | **Redis缓存接入**~~ | ~~.env已有REDIS配置，Docker已部署Redis~~ | 性能 | 中 | 否 | 否 | ✅ 已完成（2026-06-22） |
...
### TASK-005：补齐Joi校验 — ✅ 已完成（2026-06-22）
...
### TASK-008：Token撤销机制 — ✅ 已完成（2026-06-22）
